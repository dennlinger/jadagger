        ...
        for i_dagger in xrange(25):
            ...
            for step in range(100000):
                ...
                if (step % 1000 == 0):
                    ...
                    if (loss_l2.eval(feed_dict={x:obs_data, yhot:act_data}) > best_loss):
                        break
                    else:
                        best_loss = loss_l2.eval(feed_dict={x:obs_data, yhot:act_data})
...
#            obs_data = np.concatenate((expert_obs, np.array(observations)), axis=0)
#            act_data = np.concatenate((expert_act, np.squeeze(np.array(act_new))), axis=0)
